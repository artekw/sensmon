    ...
    <div class="row">
      ...
      <div class="col-sm-4">
        <div class="panel panel-primary">
          <div class="panel-heading"><i class="fa fa-tachometer" aria-hidden="true"></i> {{! v.title}}
            <!-- battery levels -->
            <span class="pull-right">
            <i ng-class="{'fa fa-plug': v.sensors.batvol.raw<=6.0 || v.sensors.batvol.raw<=4.1,
                          'fa fa-battery-full': v.sensors.batvol.raw<=4.2 || v.sensors.batvol.raw<=3.9,
            ...
            <!-- Table -->
              <table class="table">
              <th ng-repeat="(i,j) in v.sensors|nodate|nobatvol"><h5><a href="/graphs/{{! k}}/{{! i}}/day" title="Wykres">{{! j.desc}}</a></h5></th>
                <tr>
                  <!-- cels -->
                <td ng-repeat="(i,j) in v.sensors|nodate|nobatvol"
                    ng-class="{danger: i=='batvol' && j.raw<3.5 || i=='temp' && j.raw<-10,
          ...
          <div class="panel-footer panel-primary"><i class="fa fa-refresh fa-spin fa fa-fw"></i> <b>{{! v.sensors.timestamp.raw|parsedate }}</b></div>
        </div>
      </div>
    </div>
    </div>
